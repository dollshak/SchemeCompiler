...
"
  ...
  "
	...
	let pair_ops =
			let apply_impl = make_routine "apply" apply_body in
			let car_impl = make_routine "car" car_body in
			let cdr_impl = make_routine "cdr" cdr_body in
			let cons_impl = make_routine "cons" cons_body in
			let setcar_impl = make_routine "setcar" setcar_body in
			let setcdr_impl = make_routine "setcdr" setcdr_body in
      "\n" ^ apply_impl ^ "\n" ^ car_impl ^ "\n" ^ cdr_impl ^ "\n" ^ cons_impl ^ "\n" ^ setcar_impl ^ "\n" ^ setcdr_impl ^ "\n\n"
